...
local function OnPlayerSelectedArea(event)
  if event.item == "winch-tool" then
    ...
    if wagon_height_error and #selected_loaded_wagons == 0 and #selected_empty_wagons == 0 then
      ...
      player.create_local_flying_text{text={"vehicle-wagon2.elevated-rail-error"}, position=message_target.position}
      return
    end
    ...
    if event.name == defines.events.on_player_selected_area then
      ...
      if #selected_vehicles > 1 or (#selected_empty_wagons + #selected_loaded_wagons) > 1 then
        message_target = selected_vehicles[1] or selected_empty_wagons[1] or selected_loaded_wagons[1]
        player.create_local_flying_text{text={"vehicle-wagon2.too-many-selected-error"}, position=message_target.position}
        return
      end
    end
    ...
    if selected_loaded_wagons[1] then
      ...
      if loaded_wagon.get_driver() then
        ...
        player.create_local_flying_text{text={"vehicle-wagon2.wagon-passenger-error"}, position=loaded_wagon.position}

      elseif loaded_wagon.train.speed ~= 0 then
        -- Can't unload while train is moving
        player.create_local_flying_text{text={"vehicle-wagon2.train-in-motion-error"}, position=loaded_wagon.position}
      ...
      elseif not storage.wagon_data[unit_number] then
        ...
        player.create_local_flying_text{text={"vehicle-wagon2.data-error", unit_number}, position=loaded_wagon.position}
      ...
      elseif in_space and not (
        ...
        player.create_local_flying_text{text={"vehicle-wagon2.train-in-space-error", prototypes.entity[storage.wagon_data[unit_number].name].localised_name}, position=loaded_wagon.position}
      ...
      elseif check_GCKI and storage.wagon_data[unit_number].GCKI_data and storage.wagon_data[unit_number].GCKI_data.locker and
        ...
        if locker and locker.valid then
          ...
          player.create_local_flying_text{text={"vehicle-wagon2.unload-locked-vehicle-error", vehicle_prototype.localised_name, locker.name}, position=loaded_wagon.position}
        else
          -- Player no longer exists.  Should permission still apply?
          player.create_local_flying_text{text={"vehicle-wagon2.unload-locked-vehicle-error", vehicle_prototype.localised_name, "Player #"..tostring(storage.wagon_data[unit_number].GCKI_data.locker)}, position=loaded_wagon.position}
        end
      ...
      elseif check_GCKI and storage.wagon_data[unit_number].GCKI_data and storage.wagon_data[unit_number].GCKI_data.owner and
        ...
        if owner and owner.valid then
          -- Player exists, display their name
          player.create_local_flying_text{text={"vehicle-wagon2.unload-owned-vehicle-error", vehicle_prototype.localised_name, owner.name}, position=loaded_wagon.position}
        else
          -- Player no longer exists.  Should permission still apply?
          player.create_local_flying_text{text={"vehicle-wagon2.unload-owned-vehicle-error", vehicle_prototype.localised_name, "Player #"..tostring(storage.wagon_data[unit_number].GCKI_data.owner)}, position=loaded_wagon.position}
        end
      ...
      elseif storage.action_queue[unit_number] and storage.action_queue[loaded_wagon.unit_number].status ~= "spider-load" then
        ...
        player.create_local_flying_text{text={"vehicle-wagon2.loaded-wagon-busy-error"}, position=loaded_wagon.position}
    ...
    if selected_vehicles[1] then
      ...
      if not storage.vehicleMap[vehicle.name] then
        player.create_local_flying_text{text={"vehicle-wagon2.unknown-vehicle-error", vehicle.localised_name}, position=vehicle.position}

      elseif get_driver_or_passenger(vehicle) then
        player.create_local_flying_text{text={"vehicle-wagon2.vehicle-passenger-error"}, position=vehicle.position}

      elseif is_vehicle_moving(vehicle) then
        player.create_local_flying_text{text={"vehicle-wagon2.vehicle-in-motion-error"}, position=vehicle.position}
      ...
      elseif in_space and not ((vehicle.type == "spider-vehicle") or string.find(vehicle.name, "se-space", 1, true) ) then
        ...
        player.create_local_flying_text{text={"vehicle-wagon2.vehicle-in-space-error", vehicle.localised_name}, position=vehicle.position}

      elseif locker and locker ~= player then
        -- Can't load someone else's locked vehicle
        player.create_local_flying_text{text={"vehicle-wagon2.load-locked-vehicle-error", vehicle.localised_name, locker.name}, position=vehicle.position}

      elseif owner and owner ~= player then
        -- Can't load someone else's vehicle
        player.create_local_flying_text{text={"vehicle-wagon2.load-owned-vehicle-error", vehicle.localised_name, owner.name}, position=vehicle.position}
    ...
    if selected_empty_wagons[1] then
      ...
      if wagon.train.speed ~= 0 then
        ...
        player.create_local_flying_text{text={"vehicle-wagon2.train-in-motion-error"}, position=wagon.position}
      elseif (storage.player_selection[index] and
        ...
        if not vehicle or not vehicle.valid then
          ...
          player.create_local_flying_text{text={"vehicle-wagon2.vehicle-invalid-error"}, position=wagon.position}
        elseif get_driver_or_passenger(vehicle) then
          -- Selected vehicle has an occupant
          clearSelection(index)
          player.create_local_flying_text{text={"vehicle-wagon2.vehicle-passenger-error"}, position=wagon.position}
        elseif storage.action_queue[wagon.unit_number] and storage.action_queue[wagon.unit_number].status ~= "spider-load" then
          ...
          player.create_local_flying_text{text={"vehicle-wagon2.empty-wagon-busy-error"}, position=wagon.position}
        elseif distance(wagon.position, vehicle.position) > LOADING_DISTANCE then
          player.create_local_flying_text{text={"vehicle-wagon2.wagon-too-far-away-error", vehicle.localised_name}, position=wagon.position}
        else
          local loaded_name = storage.vehicleMap[vehicle.name]
          if not loaded_name then
            player.create_local_flying_text{text={"vehicle-wagon2.unknown-vehicle-error", vehicle.localised_name}, position=wagon.position}
            clearSelection(index)
          else
        ...
        player.create_local_flying_text{text={"vehicle-wagon2.no-vehicle-selected"}, position=wagon.position}
      end
    end
    ...
    if (#selected_vehicles == 0 and #selected_loaded_wagons == 0 and #selected_empty_wagons == 0) and
      ...
      if storage.action_queue[unit_number] then
        -- This wagon already has a pending action
        player.create_local_flying_text{text={"vehicle-wagon2.loaded-wagon-busy-error"}, position=wagon.position}
      elseif not unload_position then
        -- Game could not find open position to unload
        player.create_local_flying_text{text={"vehicle-wagon2.vehicle-not-created-error", prototypes.entity[storage.wagon_data[unit_number].name].localised_name}, position=position}
      elseif click_distance > max_distance then
        -- Player clicked too far away
        player.create_local_flying_text{text={"vehicle-wagon2.location-too-far-away-error", wagon.localised_name}, position=position}
      elseif click_distance < min_distance then
        player.create_local_flying_text{text={"vehicle-wagon2.location-too-close-error", wagon.localised_name}, position=position}  -- Player clicked too close
      else
